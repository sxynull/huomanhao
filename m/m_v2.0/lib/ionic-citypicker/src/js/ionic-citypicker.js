...
app.directive('ionicCityPicker', ['$ionicPopup', '$timeout','CityPickerService','$ionicScrollDelegate','$ionicModal', function ($ionicPopup, $timeout,CityPickerService, $ionicScrollDelegate,$ionicModal) {
  ...
  return {
    restrict: 'AE',
    template:' <div class="account-list border_b_1 clearfix"><span class="account-list-left">{{vm.title}}</span><span class="account-list-right">{{vm.areaData}}<i class="icon ion-ios-arrow-right"></i></span> </div>',
   // template:  '<div class={{vm.cssClass}}><i class={{vm.iconClass}}></i>{{vm.title}}<span class="item-note account-list-right">{{vm.areaData}}</span></div></div>',
    scope: {
      options : '=options'
    },
    link: function (scope, element, attrs) {
        ...
        vm.cssClass = 'ionic-citypicker item account-list'  + (angular.isDefined(so.iconClass) ? ' item-icon-left ' : ' ') + (angular.isDefined(so.cssClass) ? so.cssClass : '')
