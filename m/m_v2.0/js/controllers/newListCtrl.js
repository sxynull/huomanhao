...
app.controller('newListCtrl',function ($scope, $http,$ionicScrollDelegate,Signature,$rootScope) {
  document.title="货蛮好网-新人专享";
  ...
  $scope.ads=[];
  $scope.adsUrl=Signature.login("/advertisement?model_code=mobile_newer_activity_code");
  ...
  // 创建一些scope变量
    ...
    $scope.getRestaurants = function () {
      $scope.page++;  // 页数++
      $scope.url = Signature.login("/newer/items?sort_type=" + $scope.sort_type + "&sort_enum=" + $scope.sort_enum + "&current_page="+$scope.page+"&page_size=10");
