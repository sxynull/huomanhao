 ...
 */
'use strict';
app.controller('addvalueservicesAddshowpicCtrl',['$scope','$rootScope','$http','$ionicModal','$location','$interval','Signature','$filter',function($scope,$rootScope,$http,$ionicModal,$location,$interval,Signature,$filter){
  document.title="货蛮好网-晒图好评追评";
  ...
  //ionicModal弹窗，由下往�?;
  $ionicModal.fromTemplateUrl('templates/modaladdshowpic.html', {
    scope: $scope
  }).then(function(modal) {
    $scope.modal = modal;
  });
  ...
  var keywords=$rootScope.items.keyword;
  $scope.keywordsArr=[];
  $scope.keywordsArr=keywords.split('@@');
  var len=$scope.keywordsArr.length;
  var leni=parseInt(len*Math.random())
  $scope.key=$scope.keywordsArr[leni];
   if($rootScope.items.item_b_o.shop_name!=null){
  ...
  $scope.toLink=function () {
    ...
    if ($rootScope.items.audit_status==2) {
      $scope.pullUrl = Signature.login("/user/progress/update?activityId=" + activityId + "&stepType=21&imgUrl1="+$scope.imgurl1+"");
    }else {
      $scope.pullUrl = Signature.login("/user/progress/submitProgress?activityId=" + activityId + "&stepType=21&imgUrl1="+$scope.imgurl1+"");
    }
    $scope.noRepeata+=1;
  ...
  $scope.checkimg1=function () {
    if ($("#preview121").attr("src") === '../../m/m_v2.0/img/apply/upload_photo.png') {
      $scope.flog1 = true;
      $scope.flog2 = false;
    } else {
      $scope.flog1 = false;
  ...
  $("#imgUrl0121").on("change", function (fileDom) {
    imgFileUp(fileDom,$location.$$search.activityId,"preview121","imgurl1");
  });
