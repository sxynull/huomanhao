...
app.controller('searchResultsCtrl', ['$scope','$http','Signature','$ionicScrollDelegate','$location',function($scope,$http,Signature,$ionicScrollDelegate,$location) {
  document.title="货蛮好网-搜索结果页";
  $scope.sort_type = 1;      //排序方式
    ...
    $scope.getRestaurants = function () {
      $scope.page++;  // 页数++
      $scope.url = Signature.login("/items?sort_type=" + $scope.sort_type + "&sort_enum=" + $scope.sort_enum + "&current_page="+$scope.page+"&page_size=10&item_name="+$location.$$search.item_name);
