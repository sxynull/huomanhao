 ...
 */
'use strict';
app.controller('fundallCtrl',function($scope,$http,Signature){
  document.title="货蛮好网-资金明细";
  /*转为时间戳*/
  $scope.dateFunction=function(date){
    return Signature.DateFuc(date);
  };
  var deal_type="";
  $(".fundtype li").click(function() {
    $(".fundtype li").eq($(this).index()).addClass("current").siblings().removeClass('current');
    if($(this).index()==0){
      deal_type="";
  ...
  $scope.pulldowngetdatafund1=function(){
    $scope.current_page++;  // 页数++
      $scope.dataUrl=Signature.login("/wealth/bill/record/amount?deal_type="+deal_type+"&current_page="+$scope.current_page+"&page_size=10");
  ...
  $scope.pulldowngetdatafund=function(){
    ...
    $scope.dataUrl=Signature.login("/wealth/bill/record/amount?deal_type="+deal_type+"&current_page=1&page_size=10");
...
// 加载金额
  $scope.pulldowngetdatafund2=function(){
    $scope.dataUrl=Signature.login("/wealth/bill/record/money/data?deal_type="+deal_type+"&current_page=1&page_size=10");
    console.log($scope.dataUrl)
    $http.get($scope.dataUrl).then(function successCallback(response) {
        if(response.data.code==10000){
          $scope.balance=response.data.data.disposable_amount;
  ...
  $scope.jumpUrl=function(){
    ...
    $http.get($scope.dataUrl).then(function successCallback(response) {
        if(response.data.code==10000){
         if(response.data.data.pay_account!=undefined&&response.data.data.pay_account!=''&&response.data.data.pay_account!=null){
