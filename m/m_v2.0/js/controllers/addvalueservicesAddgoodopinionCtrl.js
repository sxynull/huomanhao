 ...
 */
'use strict';
app.controller('addvalueservicesAddgoodopinionCtrl',['$scope','$rootScope','$http','$ionicModal','$location','$interval','Signature','$filter',function($scope,$rootScope,$http,$ionicModal,$location,$interval,Signature,$filter){
  document.title="货蛮好网-好评置顶追评";
  ...
  //ionicModal弹窗，由下往�?;
  $ionicModal.fromTemplateUrl('templates/modaladdgoodopinion.html', {
    scope: $scope
  }).then(function(modal) {
    $scope.modal = modal;
  });
  ...
  var showPics=$rootScope.items.praise_text2_url;
  $scope.showPicArr=[];
  ...
  $scope.toLink=function () {
    ...
    if ($rootScope.items.audit_status==2) {
      $scope.pullUrl = Signature.login("/user/progress/update?activityId=" + activityId + "&stepType=17&imgUrl1="+$scope.imgurl1+"");
    }else {
      $scope.pullUrl = Signature.login("/user/progress/submitProgress?activityId=" + activityId + "&stepType=17&imgUrl1="+$scope.imgurl1+"");
    }
    $scope.noRepeata+=1;
  ...
  $scope.checkimg1=function () {
    if ($("#preview131").attr("src") === '../../m/m_v2.0/img/apply/upload_photo.png') {
      $scope.flog1 = true;
      $scope.flog2 = false;
    } else {
      $scope.flog1 = false;
  ...
  $("#imgUrl0131").on("change", function (fileDom) {
    imgFileUp(fileDom,$location.$$search.activityId,"preview131","imgurl1");
  });
