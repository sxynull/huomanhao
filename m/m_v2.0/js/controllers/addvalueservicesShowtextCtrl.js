 ...
 */
'use strict';
app.controller('addvalueservicesShowtextCtrl',['$scope','$rootScope','$http','$ionicModal','$location','$interval','Signature','$filter',function($scope,$rootScope,$http,$ionicModal,$location,$interval,Signature,$filter){
  document.title="货蛮好网-文字好评";
  ...
  //ionicModal弹窗，由下往�?;
  $ionicModal.fromTemplateUrl('templates/modalshowtext.html', {
    scope: $scope
  }).then(function(modal) {
    $scope.modal = modal;
  });
  ...
  $scope.toLink=function () {
    ...
    if ($rootScope.items.audit_status==2) {
      $scope.pullUrl = Signature.login("/user/progress/update?activityId=" + activityId + "&stepType=14&imgUrl1="+$scope.imgurl1+"");
    }else {
      $scope.pullUrl = Signature.login("/user/progress/submitProgress?activityId=" + activityId + "&stepType=14&imgUrl1="+$scope.imgurl1+"");
    }
    $scope.noRepeata+=1;
  ...
  $scope.checkimg1=function () {
    if ($("#preview71").attr("src") === '../../m/m_v2.0/img/apply/upload_photo.png') {
      $scope.flog1 = true;
      $scope.flog2 = false;
    } else {
      $scope.flog1 = false;
  ...
  $("#imgUrl071").on("change", function (fileDom) {
    imgFileUp(fileDom,$location.$$search.activityId,"preview71","imgurl1");
  });
