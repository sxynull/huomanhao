 ...
 */
'use strict';
app.controller('applyWangwangchatCtrl',['$scope','$rootScope','$http','$ionicModal','$location','$interval','Signature','$filter',function($scope,$rootScope,$http,$ionicModal,$location,$interval,Signature,$filter){
  document.title="货蛮好网-旺旺聊天";
  ...
  //ionicModal弹窗，由下往�?;
  $ionicModal.fromTemplateUrl('templates/modalwangwang.html', {
    scope: $scope
  }).then(function(modal) {
    $scope.modal = modal;
  });
  ...
  $scope.toLink=function () {
    ...
    if($scope.noRepeata>1){
      ...
      $scope.pullUrl=Signature.login("/user/progress/submitProgress?activityId="+activityId+"&stepType=8&imgUrl1="+$scope.imgurl1+"");
  ...
  $scope.checkimg1=function () {
    if ($("#preview61").attr("src") == '../../m/m_v2.0/img/apply/upload_photo.png') {
      $scope.flog1 = true;
      $scope.flog2 = false;
    } else {
      $scope.flog1 = false;
  ...
  $("#imgUrl061").on("change", function (fileDom) {
    imgFileUp(fileDom,$location.$$search.activityId,"preview61","imgurl1");
  });
