 ...
 */
'use strict';
app.controller('applyCollectCtrl', ['$scope', '$rootScope', '$ionicPopup', '$http','$ionicModal', '$location', '$interval', 'Signature', '$filter', function ($scope, $rootScope, $ionicPopup, $http,$ionicModal, $location, $interval, Signature, $filter) {
  document.title="货蛮好网-双收藏任务";
  ...
  //ionicModal弹窗，由下往�?;
  $ionicModal.fromTemplateUrl('templates/modalcollect.html', {
    scope: $scope
  }).then(function(modal) {
    $scope.modal = modal;
  });
  ...
  $ionicModal.fromTemplateUrl('templates/modalcollect1.html', {
    scope: $scope
  }).then(function(modal) {
    $scope.modal1 = modal;
  });
  ...
  $scope.checkimg1=function () {
    if ($("#preview11").attr("src") === '../../m/m_v2.0/img/apply/upload_photo.png') {
      $scope.flog1 = true;
      $scope.flog2 = false;
    } else {
      $scope.flog1 = false;
  ...
  $scope.checkimg2=function (){
    if ($("#preview12").attr("src") === '../../m/m_v2.0/img/apply/upload_photo.png') {
      $scope.flog4 = true;
      $scope.flog5 = false;
    } else {
      $scope.flog4 = false;
  ...
  $scope.setbackgroundColor = function (flog3, flog6) {
    var p = "";
    if (flog3 == false || flog6 == false) {
      p = '#666666';
    } else {
      p = '#ff366f';
    }
  ...
  $scope.toLink = function () {
    ...
    $scope.pullUrl = Signature.login("/user/progress/submitProgress?activityId=" + activityId + "&stepType=5&imgUrl1=" + $scope.imgurl1 + "&imgUrl2=" + $scope.imgurl2 + "");
  ...
  $("#imgUrl011").on("change", function (fileDom) {
   imgFileUp(fileDom,$location.$$search.activityId,"preview11","imgurl1");
  });
  $("#imgUrl012").on("change", function (fileDom) {
    imgFileUp(fileDom,$location.$$search.activityId,"preview12","imgurl2");
  });
